<?php
/**
 * Admin Invoice PDF / Print View
 * Generates a print-friendly HTML page that admins can save as PDF via browser print dialog.
 */
...
// Load payments
try {
    $ps = $pdo->prepare("SELECT * FROM invoice_payments WHERE invoice_id=? AND status='completed' ORDER BY created_at ASC");
    $ps->execute([$id]);
...
$amountPaid = !empty($payments) ? array_sum(array_column($payments, 'amount')) : (float)($invoice['amount_paid'] ?? 0);
...
<div class="invoice-wrapper">
    ...
    <div class="totals-section">
        <table class="totals">
            <?php if ((float)($invoice['subtotal'] ?? 0) > 0): ?>
            <tr>
                <td>Subtotal</td>
