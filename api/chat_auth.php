...
if($action==='register'){
    ...
    try{
        ...
        $pdo->prepare("INSERT INTO chat_rooms(name,type,created_by)VALUES(?,?,?)")
            ->execute(['Support: '.htmlspecialchars($name,ENT_QUOTES,'UTF-8'),'support',$userId]);
        $roomId=(int)$pdo->lastInsertId();
        $pdo->prepare("INSERT IGNORE INTO chat_room_members(room_id,user_id)VALUES(?,?)")->execute([$roomId,$userId]);
        $pdo->prepare("INSERT INTO chat_messages(room_id,sender_id,sender_name,message)VALUES(?,NULL,'Support','Welcome! How can we help you today?')")->execute([$roomId]);
    ...
    try{
        ...
        if($user&&password_verify($pass,$user['password'])){
            $rs=$pdo->prepare("SELECT r.id FROM chat_rooms r INNER JOIN chat_room_members m ON m.room_id=r.id WHERE r.type='support' AND m.user_id=? LIMIT 1");
            $rs->execute([$user['id']]); $room=$rs->fetch();
            if(!$room){
                $pdo->prepare("INSERT INTO chat_rooms(name,type,created_by)VALUES(?,?,?)")->execute(['Support: '.$user['name'],'support',$user['id']]);
                $roomId=(int)$pdo->lastInsertId();
                $pdo->prepare("INSERT IGNORE INTO chat_room_members(room_id,user_id)VALUES(?,?)")->execute([$roomId,$user['id']]);
