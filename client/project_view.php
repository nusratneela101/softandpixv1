<?php
session_start();
require_once '../config/db.php';
require_once 'includes/auth.php';
requireClient();

$userId    = $_SESSION['user_id'];
$projectId = (int)($_GET['id'] ?? 0);

if (!$projectId) { header('Location: /client/'); exit; }

try {
    $stmt = $pdo->prepare("SELECT p.*, u.name AS developer_name FROM projects p
        LEFT JOIN users u ON u.id = p.developer_id
        WHERE p.id=? AND (p.client_id=? OR ? = 'admin')");
    $stmt->execute([$projectId, $userId, $_SESSION['user_role']]);
    $project = $stmt->fetch();
} catch (Exception $e) { $project = null; }

if (!$project) { header('Location: /client/'); exit; }

// Milestones
$milestones = [];
try {
    $ms = $pdo->prepare("SELECT * FROM project_milestones WHERE project_id=? ORDER BY due_date ASC");
    $ms->execute([$projectId]);
    $milestones = $ms->fetchAll();
} catch (Exception $e) {}

// Updates
$updates = [];
try {
    $upd = $pdo->prepare("SELECT pu.*, u.name AS author_name FROM project_updates pu
        LEFT JOIN users u ON u.id = pu.user_id
        WHERE pu.project_id=? ORDER BY pu.created_at DESC LIMIT 20");
    $upd->execute([$projectId]);
    $updates = $upd->fetchAll();
} catch (Exception $e) {}

// Files (non-private)
$files = [];
try {
    $pf = $pdo->prepare("SELECT pf.*, u.name AS uploader_name FROM project_files pf
        LEFT JOIN users u ON u.id = pf.uploaded_by
        WHERE pf.project_id=? AND (pf.is_private IS NULL OR pf.is_private=0)
        ORDER BY pf.created_at DESC");
    $pf->execute([$projectId]);
    $files = $pf->fetchAll();
} catch (Exception $e) {}

$statusColor   = ['pending'=>'warning','in_progress'=>'primary','on_hold'=>'secondary','completed'=>'success','cancelled'=>'danger'][$project['status']] ?? 'secondary';
$priorityColor = ['low'=>'success','medium'=>'warning','high'=>'danger','urgent'=>'danger'][$project['priority'] ?? 'medium'] ?? 'secondary';

// Unread notifications for badge
$unreadNotifs = 0;
try {
    $un = $pdo->prepare("SELECT COUNT(*) FROM notifications WHERE user_id=? AND is_read=0");
    $un->execute([$userId]); $unreadNotifs = (int)$un->fetchColumn();
} catch (Exception $e) {}

function timeAgo($datetime) {
    $time = time() - strtotime($datetime);
    if ($time < 60) return 'just now';
    if ($time < 3600) return floor($time/60).' min ago';
    if ($time < 86400) return floor($time/3600).' hr ago';
    return floor($time/86400).' days ago';
}

function h2($str) { return htmlspecialchars($str ?? '', ENT_QUOTES, 'UTF-8'); }
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title><?php echo h2($project['title']); ?> - Client Portal</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.8.1/font/bootstrap-icons.css" rel="stylesheet">
<style>
body { background: #f8fafc; }
.sidebar { background:linear-gradient(180deg,#1e3a5f,#2563eb); width:240px; min-height:100vh; position:fixed; top:0; left:0; z-index:100; }
.sidebar .brand { padding:20px; border-bottom:1px solid rgba(255,255,255,.2); }
.sidebar .nav-link { color:rgba(255,255,255,.85); padding:10px 20px; display:flex; align-items:center; gap:10px; }
.sidebar .nav-link:hover { background:rgba(255,255,255,.15); color:#fff; border-radius:8px; margin:2px 8px; padding:10px 12px; }
.main-content { margin-left:240px; padding:24px; }
.section-card { border-radius:12px; border:1px solid #e2e8f0; background:#fff; }
.milestone-item { border-left:3px solid #dee2e6; padding:8px 12px; margin-bottom:8px; border-radius:0 6px 6px 0; }
.milestone-item.completed { border-color:#10b981; background:#f0fdf4; }
</style>
</head>
<body>
<div class="sidebar">
    <div class="brand">
        <img src="/assets/img/SoftandPix -LOGO.png" alt="" style="max-height:35px;filter:brightness(10);">
    </div>
    <nav class="nav flex-column mt-2">
        <a class="nav-link" href="/client/"><i class="bi bi-speedometer2"></i> Dashboard</a>
        <a class="nav-link" href="/client/invoices.php"><i class="bi bi-receipt"></i> Invoices</a>
        <a class="nav-link" href="/client/chat.php"><i class="bi bi-chat-dots"></i> Chat</a>
        <a class="nav-link" href="/client/notifications.php">
            <i class="bi bi-bell"></i> Notifications
            <?php if ($unreadNotifs > 0): ?>
            <span class="badge bg-warning text-dark ms-auto"><?php echo $unreadNotifs; ?></span>
            <?php endif; ?>
        </a>
        <a class="nav-link" href="/profile.php"><i class="bi bi-person"></i> Profile</a>
        <hr style="border-color:rgba(255,255,255,.2);margin:8px 16px;">
        <a class="nav-link" href="/logout.php"><i class="bi bi-box-arrow-right"></i> Logout</a>
    </nav>
</div>

<div class="main-content">
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="/client/">Dashboard</a></li>
            <li class="breadcrumb-item active"><?php echo h2($project['title']); ?></li>
        </ol>
    </nav>

    <!-- Project Header -->
    <div class="section-card p-4 mb-4">
        <div class="d-flex align-items-center gap-2 mb-2">
            <h4 class="fw-bold mb-0"><?php echo h2($project['title']); ?></h4>
            <span class="badge bg-<?php echo $statusColor; ?>"><?php echo ucwords(str_replace('_',' ',$project['status'])); ?></span>
            <?php if (!empty($project['priority'])): ?>
            <span class="badge bg-<?php echo $priorityColor; ?> bg-opacity-75"><?php echo ucfirst($project['priority']); ?> priority</span>
            <?php endif; ?>
        </div>
        <?php if (!empty($project['description'])): ?>
        <p class="text-muted"><?php echo nl2br(h2($project['description'])); ?></p>
        <?php endif; ?>

        <div class="row g-3 mt-1">
            <?php if (!empty($project['developer_name'])): ?>
            <div class="col-auto">
                <small class="text-muted">Assigned Developer</small>
                <div class="fw-semibold"><?php echo h2($project['developer_name']); ?></div>
            </div>
            <?php endif; ?>
            <?php if ($project['start_date']): ?>
            <div class="col-auto">
                <small class="text-muted">Start Date</small>
                <div class="fw-semibold"><?php echo date('M j, Y', strtotime($project['start_date'])); ?></div>
            </div>
            <?php endif; ?>
            <?php if ($project['deadline']): ?>
            <div class="col-auto">
                <small class="text-muted">Deadline</small>
                <div class="fw-semibold <?php echo strtotime($project['deadline']) < time() && $project['status'] !== 'completed' ? 'text-danger' : ''; ?>">
                    <?php echo date('M j, Y', strtotime($project['deadline'])); ?>
                </div>
            </div>
            <?php endif; ?>
        </div>

        <!-- Progress Bar -->
        <div class="mt-4">
            <div class="d-flex justify-content-between mb-1">
                <span class="fw-semibold">Overall Progress</span>
                <span class="fw-bold text-primary"><?php echo (int)($project['progress']??0); ?>%</span>
            </div>
            <div class="progress" style="height:12px;border-radius:8px;">
                <div class="progress-bar bg-<?php echo $statusColor; ?>"
                     style="width:<?php echo (int)($project['progress']??0); ?>%;"></div>
            </div>
        </div>

        <?php if (!empty($project['demo_enabled']) && !empty($project['demo_subdomain'])): ?>
        <div class="mt-4 pt-3 border-top">
            <div class="d-flex align-items-center gap-3 flex-wrap">
                <a href="https://<?php echo h2($project['demo_subdomain']); ?>.softandpix.com"
                   target="_blank" rel="noopener"
                   class="btn btn-info text-white">
                    <i class="bi bi-play-circle me-2"></i>🔗 View Live Demo
                </a>
                <?php if (!empty($project['demo_expires_at'])): ?>
                <span class="small text-muted">
                    <i class="bi bi-calendar me-1"></i>Demo expires:
                    <strong><?php echo date('M j, Y', strtotime($project['demo_expires_at'])); ?></strong>
                </span>
                <?php endif; ?>
                <?php if (!empty($project['demo_password'])): ?>
                <span class="badge bg-warning text-dark"><i class="bi bi-lock-fill me-1"></i>Password Protected</span>
                <?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
    </div>

    <div class="row g-4">
        <div class="col-lg-8">
            <!-- Milestones -->
            <?php if (!empty($milestones)): ?>
            <div class="section-card p-4 mb-4">
                <h6 class="fw-bold mb-3"><i class="bi bi-flag me-2 text-primary"></i>Milestones</h6>
                <?php foreach ($milestones as $m): $mDone = $m['status'] === 'completed'; ?>
                <div class="milestone-item <?php echo $mDone ? 'completed' : ''; ?> d-flex justify-content-between align-items-center">
                    <div>
                        <div class="fw-semibold <?php echo $mDone ? 'text-decoration-line-through text-muted' : ''; ?>">
                            <?php echo h2($m['title']); ?>
                        </div>
                        <?php if ($m['due_date']): ?><small class="text-muted">Due: <?php echo date('M j, Y',strtotime($m['due_date'])); ?></small><?php endif; ?>
                    </div>
                    <span class="badge bg-<?php echo $mDone ? 'success' : 'secondary'; ?>"><?php echo $mDone ? 'Done' : ucfirst($m['status']); ?></span>
                </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>

            <!-- Activity Feed -->
            <?php if (!empty($updates)): ?>
            <div class="section-card p-4">
                <h6 class="fw-bold mb-3"><i class="bi bi-activity me-2 text-primary"></i>Project Updates</h6>
                <?php foreach ($updates as $u): ?>
                <div class="d-flex gap-3 mb-3">
                    <div class="flex-shrink-0">
                        <div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center"
                             style="width:36px;height:36px;font-size:.85rem;">
                            <?php echo strtoupper(substr($u['author_name'] ?? 'T', 0, 1)); ?>
                        </div>
                    </div>
                    <div>
                        <div class="fw-semibold small"><?php echo h2($u['author_name'] ?? 'Team'); ?>
                            <span class="text-muted fw-normal ms-1"><?php echo timeAgo($u['created_at']); ?></span>
                        </div>
                        <div class="mt-1 text-muted"><?php echo nl2br(h2($u['message'])); ?></div>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
        </div>

        <!-- Files -->
        <div class="col-lg-4">
            <div class="section-card p-4">
                <h6 class="fw-bold mb-3"><i class="bi bi-folder2 me-2 text-primary"></i>Deliverables & Files</h6>
                <?php if (empty($files)): ?>
                <p class="text-muted small">No files available yet.</p>
                <?php else: ?>
                <?php foreach ($files as $f):
                    $ext = strtolower(pathinfo($f['file_name'] ?? $f['file_path'] ?? '', PATHINFO_EXTENSION));
                    $icon = in_array($ext,['jpg','jpeg','png','gif','webp']) ? 'bi-image' :
                            ($ext === 'pdf' ? 'bi-file-pdf' : ($ext === 'zip' ? 'bi-file-zip' : 'bi-file-earmark'));
                ?>
                <div class="d-flex align-items-center gap-2 mb-2 p-2 border rounded">
                    <i class="bi <?php echo $icon; ?> text-primary fs-5"></i>
                    <div class="flex-grow-1 overflow-hidden">
                        <div class="small fw-semibold text-truncate"><?php echo h2($f['file_name'] ?? basename($f['file_path'] ?? 'File')); ?></div>
                        <div class="text-muted" style="font-size:.72rem;"><?php echo timeAgo($f['created_at']); ?></div>
                    </div>
                    <a href="<?php echo h2($f['file_path']); ?>" target="_blank" class="btn btn-sm btn-outline-secondary">
                        <i class="bi bi-download"></i>
                    </a>
                </div>
                <?php endforeach; ?>
                <?php endif; ?>
            </div>
        </div>
    </div>
</div>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
