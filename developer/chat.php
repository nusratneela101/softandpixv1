...
$userName   = htmlspecialchars($_SESSION['user_name'] ?? 'Developer', ENT_QUOTES, 'UTF-8');
...
try {
    ...
    foreach ($convRows as &$cr) {
        if (!empty($cr['title'])) {
            ...
            $ps = $pdo->prepare(
                "SELECT COALESCE(u.name, IF(cp.user_id=0,'Admin','User')) AS n
...
<title>Chat - Developer Portal</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.8.1/font/bootstrap-icons.css" rel="stylesheet">
<style>
*{box-sizing:border-box;}
body{background:#f8fafc;height:100vh;overflow:hidden;margin:0;}
.sidebar{background:linear-gradient(180deg,#1e293b,#334155);width:220px;min-height:100vh;position:fixed;top:0;left:0;z-index:100;display:flex;flex-direction:column;}
...
.chat-header{background:#1e293b;color:#fff;padding:13px 18px;display:flex;align-items:center;gap:12px;flex-shrink:0;}
...
.chat-msg-mine{background:#16a34a;color:#fff;border-bottom-right-radius:4px;}
...
<div class="sidebar">
    ...
    <nav class="nav flex-column mt-2">
        ...
        <a class="nav-link active" href="/developer/chat.php"><i class="bi bi-chat-dots"></i> Chat</a>
        <a class="nav-link" href="/developer/notifications.php"><i class="bi bi-bell"></i> Notifications</a>
        <a class="nav-link" href="/developer/deadline_request.php"><i class="bi bi-calendar-x"></i> Deadline Req.</a>
...
<div class="chat-wrapper">
    ...
    <div class="chat-panel">
        ...
        <div class="chat-input-bar">
            ...
            <button class="btn btn-success px-3" id="sendBtn"><i class="bi bi-send"></i></button>
        </div>
        <?php else: ?>
        <div class="no-conv">
