...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'update_profile') {
        ...
        $portfolio_url  = trim(htmlspecialchars($_POST['portfolio_url'] ?? '', ENT_QUOTES, 'UTF-8'));
        $specialization = trim(htmlspecialchars($_POST['specialization'] ?? '', ENT_QUOTES, 'UTF-8'));
        ...
        try {
            ...
            // Optional / newer columns — whitelist prevents injection, catch unknown column only
            $allowedOptCols = ['timezone', 'website', 'social_github', 'social_linkedin', 'social_twitter',
                               'portfolio_url', 'specialization', 'availability_status'];
            $optColValues   = [$timezone, $website, $social_github, $social_linkedin, $social_twitter,
                               $portfolio_url, $specialization, $availability];
...
foreach (['website','city','timezone','social_github','social_linkedin','social_twitter','specialization','availability_status','portfolio_url'] as $col) {
...
try {
    $sp = $pdo->prepare("SELECT status FROM projects WHERE developer_id=? AND status != 'cancelled'");
    $sp->execute([$userId]);
    $prows = $sp->fetchAll();
    $totalProjects = count($prows);
    $activeProjects = count(array_filter($prows, fn($p) => $p['status'] === 'in_progress'));
    $completedProjects = count(array_filter($prows, fn($p) => $p['status'] === 'completed'));
} catch (Exception $e) {}
...
$availLabels = ['available' => ['label' => 'Available', 'color' => 'success'],
                'busy'      => ['label' => 'Busy',      'color' => 'warning'],
                'on_leave'  => ['label' => 'On Leave',  'color' => 'secondary']];
$avail = $availLabels[$user['availability_status'] ?: 'available'] ?? $availLabels['available'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
.avatar-placeholder{width:110px;height:110px;border-radius:50%;background:linear-gradient(135deg,#0ea5e9,#0284c7);display:flex;align-items:center;justify-content:center;font-size:2.5rem;color:#fff;border:4px solid #fff;box-shadow:0 4px 16px rgba(0,0,0,.15);}
.profile-card{background:#fff;border-radius:16px;box-shadow:0 2px 12px rgba(0,0,0,.07);overflow:hidden;}
.profile-header{background:linear-gradient(135deg,#0c4a6e,#0ea5e9);padding:32px 24px 20px;text-align:center;color:#fff;}
.skill-badge{display:inline-block;background:#e0f2fe;color:#0369a1;border-radius:20px;padding:3px 12px;font-size:.8rem;margin:2px;}
...
<div class="sidebar">
    ...
    <div class="mt-3 px-3 pb-2 small text-white-50">
        ...
        <span class="badge ms-1" style="background:rgba(255,255,255,.2);"><?php echo h($role); ?></span>
    </div>
    <nav class="nav flex-column mt-2">
        ...
        <a class="nav-link active" href="/developer/profile.php"><i class="bi bi-person-circle"></i> Profile</a>
        ...
        <div class="col-lg-4">
            <div class="profile-card">
                <div class="profile-header">
                    ...
                    <p class="mb-1 small opacity-75"><?php echo h($user['email'] ?? ''); ?></p>
                    <?php if (!empty($user['specialization'])): ?>
                    <p class="mb-1 small opacity-85"><?php echo h($user['specialization']); ?></p>
                    <?php endif; ?>
                    <span class="badge mt-1" style="background:rgba(255,255,255,.2);"><?php echo h(ucfirst(str_replace('_', ' ', $role))); ?></span>
                    <span class="badge ms-1 bg-<?php echo h($avail['color']); ?>"><?php echo h($avail['label']); ?></span>
                </div>
                <div class="card-body p-4">
                    <div class="section-title">Statistics</div>
                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <span class="small text-muted"><i class="bi bi-kanban me-2"></i>Total Projects</span>
                        <span class="fw-semibold"><?php echo $totalProjects; ?></span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <span class="small text-muted"><i class="bi bi-arrow-repeat me-2"></i>In Progress</span>
                        <span class="fw-semibold"><?php echo $activeProjects; ?></span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <span class="small text-muted"><i class="bi bi-check2-circle me-2"></i>Completed</span>
                        <span class="fw-semibold"><?php echo $completedProjects; ?></span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <span class="small text-muted"><i class="bi bi-calendar me-2"></i>Joined</span>
                    ...
                    <div class="mt-3">
                        ...
                        <?php foreach ($skillsList as $sk): ?><span class="skill-badge"><?php echo h($sk); ?></span><?php endforeach; ?>
                    </div>
                    <?php endif; ?>
                    <?php
                    ...
                    <div class="mt-3">
                        ...
                        <?php if ($po): ?><a href="<?php echo h($po); ?>" target="_blank" rel="noopener" class="btn btn-sm btn-outline-dark me-1 mb-1"><i class="bi bi-briefcase"></i> Portfolio</a><?php endif; ?>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
                            ...
                            <form method="POST" enctype="multipart/form-data" id="profileForm">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="d-flex align-items-center gap-3 flex-wrap">
                                        <div>
                                            ...
                                            <?php else: ?>
                                            <div id="formAvatarPlaceholder" style="width:60px;height:60px;border-radius:50%;background:linear-gradient(135deg,#0ea5e9,#0284c7);display:flex;align-items:center;justify-content:center;font-size:1.4rem;color:#fff;"><?php echo h($initials); ?></div>
                                ...
                                <div class="row g-3 mb-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" name="skills" class="form-control" value="<?php echo h($user['skills'] ?? ''); ?>" placeholder="e.g. PHP, JavaScript, Laravel, React">
                                    </div>
                                </div>

                                <!-- Developer-specific -->
                                <div class="section-title mt-3">Developer Details</div>
                                <div class="row g-3 mb-3">
                                    <div class="col-sm-6">
                                        <label class="form-label small fw-semibold">Specialization</label>
                                        <input type="text" name="specialization" class="form-control" value="<?php echo h($user['specialization'] ?? ''); ?>" placeholder="e.g. Full Stack Developer">
                                    </div>
                                    <div class="col-sm-6">
                                        <label class="form-label small fw-semibold">Availability Status</label>
                                        <select name="availability_status" class="form-select">
                                            <?php $cur_avail = $user['availability_status'] ?: 'available'; ?>
                                            <option value="available" <?php echo $cur_avail === 'available' ? 'selected' : ''; ?>>🟢 Available</option>
                                            <option value="busy"      <?php echo $cur_avail === 'busy'      ? 'selected' : ''; ?>>🟡 Busy</option>
                                            <option value="on_leave"  <?php echo $cur_avail === 'on_leave'  ? 'selected' : ''; ?>>⚫ On Leave</option>
                                        </select>
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label small fw-semibold">Portfolio URL</label>
                                        <input type="url" name="portfolio_url" class="form-control" value="<?php echo h($user['portfolio_url'] ?? ''); ?>" placeholder="https://yourportfolio.com">
                                    </div>
                                </div>
