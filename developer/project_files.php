<?php
/**
 * developer/project_files.php
 * Developer file manager — can upload, download, comment, delete own files.
 */
...
// ── Verify access ─────────────────────────────────────────────
try {
    $stmt = $pdo->prepare("SELECT * FROM projects WHERE id = ? AND (developer_id = ? OR ? = 'admin')");
...
if ($folderId) {
    ...
    if (!$currentFolder) { header('Location: /developer/project_files.php?project_id=' . $projectId); exit; }
}
...
<!-- Top Nav -->
<nav class="top-nav">
    <a href="/developer/" class="brand"><i class="bi bi-code-slash me-2 text-primary"></i>Developer Portal</a>
    <div class="d-flex align-items-center gap-3">
        <a href="/developer/notifications.php" class="text-decoration-none position-relative">
...
<div class="container-fluid py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <h4 class="mb-0">
            ...
            Files — <a href="/developer/project_view.php?id=<?php echo $projectId; ?>" class="text-decoration-none"><?php echo h($project['title']); ?></a>
        </h4>
        <a href="/developer/project_view.php?id=<?php echo $projectId; ?>" class="btn btn-sm btn-outline-secondary">
    ...
    <div class="fm-breadcrumb mb-2">
        ...
        <a class="crumb" href="/developer/project_files.php?project_id=<?php echo $projectId; ?>">Root</a>
        <?php foreach ($breadcrumbs as $bc): ?>
            <span class="crumb-sep">/</span>
            <?php if ($bc['id'] != $folderId): ?>
                <a class="crumb" href="/developer/project_files.php?project_id=<?php echo $projectId; ?>&folder_id=<?php echo $bc['id']; ?>"><?php echo h($bc['name']); ?></a>
            <?php else: ?>
    ...
    <div id="fm-bulk-bar" class="alert alert-info d-none d-flex align-items-center gap-2 py-2 mb-2">
        ...
        <button class="btn btn-sm btn-danger" onclick="devBulkDelete()"><i class="bi bi-trash me-1"></i>Delete Mine</button>
        ...
        <div id="fm-items-container" class="fm-list p-2">
            ...
            <div class="fm-list-row" data-id="<?php echo $folder['id']; ?>" data-type="folder">
                ...
                <div class="list-actions">
                    <a href="/developer/project_files.php?project_id=<?php echo $projectId; ?>&folder_id=<?php echo $folder['id']; ?>"
            ...
            <?php foreach ($files as $file):
                ...
                $isOwn    = ((int)$file['uploaded_by'] === (int)$userId);
            ?>
            <div class="fm-list-row" data-id="<?php echo $file['id']; ?>" data-type="file"
                ...
                <span class="list-meta"><?php echo h($file['uploader_name'] ?? ''); ?></span>
                ...
                <div class="list-actions">
                    ...
                    <button class="btn btn-sm btn-outline-danger" title="Delete"
                            onclick="event.stopPropagation(); devDeleteFile(<?php echo $file['id']; ?>)">
...
var _csrf = <?php echo json_encode($csrf); ?>;

function devDeleteFile(id) {
    if (!confirm('Delete this file?')) return;
    fetch('/api/files/delete.php', {
        method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded'},
        body:'type=file&id='+encodeURIComponent(id)+'&csrf_token='+encodeURIComponent(_csrf)
    }).then(function(r){return r.json();}).then(function(res){
        if (res.success) {
            var card = document.querySelector('[data-id="'+id+'"][data-type="file"]');
            if (card) card.remove();
        } else { alert(res.error||'Delete failed.'); }
    }).catch(function(){alert('Network error.');});
}

function devBulkDelete() {
    var items = document.querySelectorAll('.fm-card.selected[data-type="file"], .fm-list-row.selected[data-type="file"]');
    var ownItems = Array.from(items).filter(function(el){ return el.dataset.own === '1'; });
    if (!ownItems.length) { alert('You can only delete your own files.'); return; }
    if (!confirm('Delete '+ownItems.length+' of your file(s)?')) return;
    ownItems.forEach(function(el){ devDeleteFile(el.dataset.id); });
}
...
function submitComment() {
    ...
    fetch('/api/files/comment.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},
        body:'file_id='+encodeURIComponent(fileId)+'&comment='+encodeURIComponent(text)+'&csrf_token='+encodeURIComponent(_csrf)
    }).then(function(r){return r.json();}).then(function(res){
