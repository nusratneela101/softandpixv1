...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $tx_id = 'STRIPE-' . time() . '-' . mt_rand(1000, 9999);
    ...
    if ($admin) { create_notification($pdo, $admin['id'], 'payment', 'Payment Received', '$' . number_format($payment['total'], 2) . ' for ' . $payment['invoice_number'], '/admin/payments.php'); }
...
<title>Pay with Stripe — SoftandPix</title>
...
<h4 class="text-center mb-4"><i class="fab fa-stripe fa-2x d-block mb-2" style="color:#6772e5"></i>Pay with Stripe</h4>
...
<div class="mb-3"><label class="form-label">Card Number</label><input type="text" class="form-control" placeholder="4242 4242 4242 4242" required></div>
<div class="row g-2 mb-3"><div class="col"><input type="text" class="form-control" placeholder="MM/YY" required></div><div class="col"><input type="text" class="form-control" placeholder="CVC" required></div></div>
<button type="submit" class="btn w-100 py-2" style="background:#6772e5;color:white"><i class="fas fa-lock me-1"></i>Pay $<?= number_format($payment['total'], 2) ?></button>
